...
class WaitAction(character: Character)
  extends TargetedAction[Character](
    character,
    EnergyCost.STANDARD(character),
    new ActionCosts(),
    new TargetEffect[Character, CharacterStat]("Waiting") {
      override def getEffect: Option[Effect[Character, CharacterStat]] = {
        new Effect(this, List(
          new Modifier[Vision](AdditiveCalculation.fixed(5.0)),
          new Modifier[Hearing](AdditiveCalculation.fixed(5.0))
        ), new Duration(Duration.VERY_SHORT))
      }
    }) {
