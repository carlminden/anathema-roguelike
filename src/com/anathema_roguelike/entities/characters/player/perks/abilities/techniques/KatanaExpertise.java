package com.anathema_roguelike.entities.characters.player.perks.abilities.techniques;

import com.anathema_roguelike.entities.characters.Character;
import com.anathema_roguelike.entities.characters.actions.attacks.WeaponAttack;
import com.anathema_roguelike.entities.characters.perks.actions.ActionEffectModificationPerk;

public class KatanaExpertise extends Technique<ActionEffectModificationPerk<Character, WeaponAttack>> {

	@Override
	protected ActionEffectModificationPerk<Character, WeaponAttack> createPerk() {
		// TODO Auto-generated method stub
		return null;
	}
}
