...
package entities.characters.player.perks.masteries
...
class MasteryLevel[T <: Mastery : TypeTag](amount :Int = 1) extends PassivePerk(Utils.getName(typeTagToClass[T])) {

  override def getEffect: Option[Buff] = new Buff(this, List(new Modifier[T](AdditiveCalculation.fixed(amount))))
}