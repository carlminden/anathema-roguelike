...
class ThrowingRange[T <: Targetable : TypeTag](weight: Character => Double,constraints: TargetConstraint[T, Character]*)
  ...
  protected def getRadius (character: Character): Double = {
    ...
    (2 + throwingMastery) * (1 + (2 * strength - weight(character)) / 30)
  }
}