...
class Pants(material: ArmorMaterial, location: Either[Location, Character])
  extends Armor(ItemPropertyCache.getProperty(classOf[ArmorType], "Pants"), material, location) with ItemType[Pants] {
  ...
  override def getVisualRepresentation = new VisualRepresentation('\u03A0', getColor)
}