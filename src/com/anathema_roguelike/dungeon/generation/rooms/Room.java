...
import com.anathema_roguelike.dungeon.generation.Feature;
import com.anathema_roguelike.main.Config;
import com.anathema_roguelike.main.ui.uielements.Rectangular;

public abstract class Room extends Feature implements Rectangular, Comparable<Room> {
	...
	public abstract void generateEncounter(DungeonLevel level);
	...
	public void place(DungeonGenerator generator) {
		...
		DungeonLevel level = generator.getLevel();
		
		for(int i = 1; i < height - 1; i++) {
			for(int j = 1; j < width - 1; j++) {
				int x = getX() + j;
				int y = getY() + i;
				map[x][y] = new Floor(level, new Point(x, y));
			}
		}
		
		for(Door door : doors) {
			map[door.getX()][door.getY()] = new Doorway(level, door.getPosition(), door.getDirection());
		}
	}
