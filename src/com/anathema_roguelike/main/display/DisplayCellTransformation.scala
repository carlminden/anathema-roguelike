...
object DisplayCellTransformation {
  def noTransformation: DisplayCellTransformation = new DisplayCellTransformation() {
    override def compute(buffer: DisplayBuffer, x: Int, y: Int, string: Char, color: SColor, display: Boolean): DisplayCell = {
      new DisplayCell(string, color, display)
    }
  }
}

abstract class DisplayCellTransformation {
