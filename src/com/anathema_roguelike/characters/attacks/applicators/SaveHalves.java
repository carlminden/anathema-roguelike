...
public class SaveHalves extends SaveNegates {
	
	@Override
	public AttackCalculation missCalculation(Attack attack, Character target, AttackCalculation attackCalculation) {
		int halfDamage = attackCalculation.getDamageResult() / 2;
		
		return new AttackCalculation(attackCalculation.getOffense(), attackCalculation.getModifier(), halfDamage);
	}
	
	@Override
	public Message missMessage(AttackResults results, Attack attack, Character target, AttackCalculation attackCalculation) {
		return new Message(target + " saves against " + Utils.getName(attack) + " taking only "
				+ attackCalculation.getDamageResult() + " points of damage", Color.LIGHT_BROWN);
	}
}
